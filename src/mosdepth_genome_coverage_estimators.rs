            ...
            CoverageEstimator::MeanGenomeCoverageEstimator {
                ...
                *total_mismatches += total_mismatches_in_contig;
                ...
                match *contig_end_exclusion * 2 < len as u32 {
                    true => *total_bases += len as u32 - 2 * *contig_end_exclusion,
                    false => {
    ...
    fn calculate_coverage(&mut self, unobserved_contig_lengths: &Vec<u32>) -> f32 {
        match self {
            ...
            CoverageEstimator::CoverageFractionGenomeCoverageEstimator {
                ...
                let final_total_bases: u32 =
                    *total_bases + unobserved_contig_lengths.iter().sum::<u32>();
                if final_total_bases == 0
                    || (*num_covered_bases as f32 / final_total_bases as f32)
                        < *min_fraction_covered_bases
                {
            ...
            CoverageEstimator::ReadsPerBaseCalculator {
                ...
                *num_mapped_reads as f32
                    / (*observed_contig_length + unobserved_contig_lengths.iter().sum::<u32>())
                        as f32
            }
        }
    }
